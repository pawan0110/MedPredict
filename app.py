...
def predict_diabetes():
    try:
        ...
        prob       = diabetes_model.predict_proba(pd.DataFrame([input_data]))[0][1]
        prediction = 1 if prob > DIABETES_THRESHOLD else 0
        result     = "Diabetic" if prediction else "Not Diabetic"
        risk_level = "Low Risk" if prob < 0.30 else ("Moderate Risk" if prob < 0.60 else "High Risk")

        return render_template("result.html", disease="Diabetes",
                               prediction=result, probability=round(prob * 100, 2),
...
def predict_heart():
    try:
        ...
        prob       = heart_model.predict_proba(pd.DataFrame([input_data]))[0][1]
        prediction = 1 if prob > HEART_THRESHOLD else 0
        result     = "Heart Disease Detected" if prediction else "No Heart Disease"
        risk_level = "Low Risk" if prob < 0.30 else ("Moderate Risk" if prob < 0.60 else "High Risk")

        return render_template("result.html", disease="Heart Disease",
                               prediction=result, probability=round(prob * 100, 2),
                               risk_level=risk_level)
    except Exception as e:
...
def predict_breast_cancer():
    try:
        ...
        input_df     = pd.DataFrame([{f: float(request.form[f]) for f in features}])
        ...
        return render_template("result.html", disease="Breast Cancer",
                               prediction=result, probability=probability_val,
                               risk_level=risk_level)
    except Exception as e:
...
# =====================================
# Kidney Disease Routes
# =====================================
def preprocess_kidney(raw: dict) -> np.ndarray:
    raw_clean = {k.strip().lower(): v for k, v in raw.items()}
    df = pd.DataFrame([raw_clean])

    df[kidney_num_cols] = kidney_num_imputer.transform(df[kidney_num_cols])
    df[kidney_cat_cols] = kidney_cat_imputer.transform(df[kidney_cat_cols])
...
def predict_kidney():
    try:
        ...
        # Map 5-class label to risk level
        risk_map = {
            'No_Disease':     'Low Risk',
            'Low_Risk':       'Low Risk',
